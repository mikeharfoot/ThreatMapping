...
for(t in taxa)
{
  ...
  #layout(matrix(1:14,ncol = 2,byrow = F),heights = c(rep(c(0.85,0.15)/4,3),1))
  layout(matrix(c(1:3,7,4:6,8),ncol = 2,byrow = F),heights = rep(0.25,4))
  ...
  for(th in primary.threats)
  {
    ...
    PlotSingleMapInMultiPanel(d = sqrt(grid@data[[colname]]),
                              ...
                              axis = F)
    mtext(side = 1,text = th,line = -2.6, adj = 0.6)
    i = i+1
  }
...
for(t in 1:3)
{
  ...
  #png(paste0(out.dir,"ThreatvsHFI_Heatmap_",names(HFI.threats)[t],".png"), width = 15, height = 15, units = "cm", res = 600)
  {
    ...
    mtext(side = 3,letters[t],cex=1.1,line = -0.5,adj = -0.2)
  }
  #dev.off()
...
par(mar=c(3,5,0,5))
PlotSingleMapInMultiPanel(d = outlier.distance[[i]],sqrt = F,
                          cell.i = gt10sp.i[[i]],i = 3+i,axis = T,axis.only = T,
                          n.breaks = 100,
                          lower.cut = 0,
                          range = c(-1,1),
                          axis.range = c(-1,1),
...
lapply(1:length(HFI.means), function(i)

  PlotSingleMap(d = outlier.distance[[i]],
                cell.i = gt10sp.i[[i]],
                n.breaks = 100,
                lower.cut = 0,
                range = c(-1,1),
                axis.range = c(-1,1),
                fname = paste0(out.dir,"ResidualDistancegt10sp_",names(HFI.threats)[i],".png"),
                color.scheme = brewer.pal(n = 9,"RdBu"))
    )
...
#### Calculate the distribution of threat probabilities for METT PAs #####

pa.fgdb<-"C:/Users/mikeha/Work/Spatial data/WDPA_May2019_Public/WDPA_May2019_Public.gdb"
fc_list <- ogrListLayers(pa.fgdb)
print(fc_list)

wdpa<-readOGR(dsn=pa.fgdb,layer="WDPA_poly_May2019")

mett<-read.csv("C:/Users/mikeha/Dropbox/Threat mapping/METT validation/IUCN_Threat_METT3_tidied.csv",stringsAsFactors = F)

mett.in.wdpa<-match(mett$WDPA.ID,wdpa@data$WDPAID)

mett.w.wdpa<-mett[(!is.na(mett.in.wdpa)),]
mett.wdpa<-wdpa[mett.in.wdpa[!is.na(mett.in.wdpa)],]

mett.wdpa<-spTransform(mett.wdpa,moll.proj)

grid.mett.wdpa<-gIntersects(mett.wdpa,grid,byid = T)

small.grid.mett.wdpa<-grid.mett.wdpa[rowSums(grid.mett.wdpa) > 0,]
small.grid.data<-grid@data[rowSums(grid.mett.wdpa) > 0,]
...
mett.w.wdpa.coded<-mett.w.wdpa[,grep("^X", colnames(mett.w.wdpa))]
mett.w.wdpa.coded[mett.w.wdpa.coded == ""]<-"Not given"
mett.w.wdpa.coded[mett.w.wdpa.coded == "BLANK"]<-"Not given"
for(i in 1:length(mett.levels))
{
  mett.w.wdpa.coded[mett.w.wdpa.coded == names(mett.levels)[i]]<-mett.levels[[i]]
}


for(i in 1:ncol(mett.w.wdpa.coded)) mett.w.wdpa.coded[,i]<-as.integer(mett.w.wdpa.coded[,i])
...
for(th in primary.threats)
{
  ...
  for(t in taxa)
  {
    ...
    p.th<-small.grid.data[[grep(paste0(t,"_",th,"gt10sp$"),colnames(grid@data), value = T)]][]
    
    #find the median P(th) for each mett pa
    mett.pa.p.th[[th]][[t]]<-apply(small.grid.mett.wdpa,2,function(x) median(p.th[x],na.rm=T))
    
    #max.mett.pa.p.th[[th]][[t]]<-apply(grid.mett.wdpa,2,function(x) max(p.th[x],na.rm=T))
    mett.pa.p.th[[th]][[t]][is.infinite(mett.pa.p.th[[th]][[t]])]<-NA
  }
}
...
q.p.th.mett.level<-list()
mn.p.th.mett.level<-list()
sd.p.th.mett.level<-list()
for(th in primary.threats)
{
  print(th)
  #first find the maximum category for each mett pa
  #find the mett columns for this threat
  mett.th.cols<-grep(paste0("X",mett.codes[[th]]),colnames(mett.w.wdpa),value = T)
  if(length(mett.th.cols) > 1)
  {
    ...
    #     u
    #   })
    agg.mett.th.levels<-apply(mett.w.wdpa.coded[,mett.th.cols],1,function(x) max(x))
  } else
  {
    agg.mett.th.levels<-mett.w.wdpa.coded[,mett.th.cols]
  }
  
  mett.pas.by.level<-lapply(list(c(2),c(4)), 
                            function(m) which(agg.mett.th.levels %in% m))
 
  for(t in taxa)
  {
    print(t)
    #p.th.mett.level[[th]][[t]]<-do.call(rbind,lapply(mett.pas.by.level,function(r) mean(mett.pa.p.th[[th]][[t]][r], na.rm=T)))
    q.p.th.mett.level[[th]][[t]]<-do.call(rbind,lapply(mett.pas.by.level,function(c) quantile(mett.pa.p.th[[th]][[t]][c],probs=c(0,0.025,0.25,0.5,0.75,0.975,1.0), na.rm=T)))
    mn.p.th.mett.level[[th]][[t]]<-do.call(rbind,lapply(mett.pas.by.level,function(c) mean(mett.pa.p.th[[th]][[t]][c], na.rm=T)))
    sd.p.th.mett.level[[th]][[t]]<-do.call(rbind,lapply(mett.pas.by.level,function(c) sd(mett.pa.p.th[[th]][[t]][c], na.rm=T)))
...
#Test if the high threat METT pas have a significantly higher threat score than the low threat
...
for(th in primary.threats)
{
  for(t in taxa)
  {
    low = mett.pa.p.th[[th]][[t]][mett.pas.by.level[[1]]]
    high = mett.pa.p.th[[th]][[t]][mett.pas.by.level[[2]]]
    boxplot(list(low = low, high = high))
    
    w.gt[[th]][[t]]<-wilcox.test(x = high,
                y = low,
                exact = F,
                alternative = "great")
    w.2sided[[th]][[t]]<-wilcox.test(x = high,
                                 y = low,
                                 exact = F,alternative = "two.sided")
  }
}

sapply(primary.threats,function(th) lapply(w.gt[[th]], function(x) x$p.value))
sapply(primary.threats,function(th) lapply(w.2sided[[th]], function(x) x$p.value))
